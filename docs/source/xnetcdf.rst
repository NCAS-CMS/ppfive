...
.. code-block:: python
   ...
   >>> u = umfive.File('test2.pp')  # Open the dataset with umfive
   >>> print(u)
   ...
   >>> x = xnetcdf.Dataset(u)  # Pass the umfive.File instance to xnetcdf
   >>> print(x)
   test2.pp: <xnetcdf.Dataset: /, 5 dimensions, 9 variables, 0 groups>
        Dimensions:
   ...
   >>> x = xnetcdf.Dataset('test2.pp')  # Pass the dataset name to xnetcdf
   >>> x
   test2.pp: <xnetcdf.Dataset: /, 5 dimensions, 9 variables, 0 groups>
